...
@dataclass
class ReloadPartitionTable:
    """
    Prepare tasks to execute the hive query using the hive cli in the edge box.

    1. customizable hive configurations are prepared and stored in the
        process configs, eg. num of mappers etc.
    2. environment parameters are programmed in the
       operator configs eg. env_name etc.
    3. hive queries must be stored in the gen_sql folder with
    ...
    def get_operator(self):
        ...
        workflow_operator = BigQueryExecuteQueryOperator(
            task_id=_task_name,
            destination_dataset_table=_projectname
            + ":"
            + _table_name
            + "$"
            + _date,
